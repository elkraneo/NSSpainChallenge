...
struct Card3DView: View {
  ...
  var body: some View {
    RealityView { content, attachments in
        ...
        let symbol = attachments.entity(for: "Symbol")
      {
        ...
        accessibilityComponent.label = LocalizedStringResource(
          stringLiteral: card.accessibilityLabel)
        accessibilityComponent.systemActions = [.activate]

        container.components[AccessibilityComponent.self] = accessibilityComponent
  ...
  private func flipCard(_ entity: Entity) {
    ...
    let spinAnimation = try! AnimationResource.makeActionAnimation(
      for: spinAction,
      duration: accessibilityReduceMotion ? 0 : 0.5,
    ...
    //2
    Task { @MainActor in
      var flippedCardLabel = AttributedString(card.accessibilityLabel)
      flippedCardLabel.accessibilitySpeechAnnouncementPriority = .high
      AccessibilityNotification.Announcement(flippedCardLabel).post()
    }
  }
}
